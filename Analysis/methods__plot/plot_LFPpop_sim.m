...
%                                 imagesc(ts,1:size(be_low_norm,2),be_low_norm'); %colorbar;
    ...
    case 'coherogram_reward_PPC_MST'
        type = 'reward'
        ev = 'stop'
        time_win =[-0.25 0.25];   % [-0.25 0.25]; [-0.45 0.25];
        freq_win = [3 51];
        align_t = 1;  % 1.3 for target (so it's aligned to target offset
        ...
        for nmonk = 1 %[1 3]
            for nsess = 1:length(monk(nmonk).coher.sess)
                ...
                for cond = 1:ncond
                    ...
                    colormap(copper);
                    imagesc(ts_win,freq(freq>freq_win(1) & freq<freq_win(2)),p_cohero_win/max_cohero); axis xy; %colorbar;
                    set(gca, 'ylim',[4 50], 'FontSize', 22); vline(0,'w'); axis square; % vline(-0.3,'b');
