...
switch plot_type
    ...
    case 'erp_move_ba'
        ...
        if strcmp(ar,'MST'), monk_ids = [1 3]; elseif strcmp(ar,'PFC'), monk_ids = [3 4]; else monk_ids = 1:4; end
        if strcmp(ev,'target'), win = [-1.3 1.3];, else, win = [-1 1]; end
        %% go through each monkey, extract and average all sessions and store
        for m = monk_ids
            ncond = length(monk(1).erp.(ar).sess(1).lfps(1).trialtype.(type));
            ts_ev = monk(m).erp.(ar).sess(1).lfps(1).trialtype.(type)(ncond).events.(ev).erp_time; ts_win = ts_ev(ts_ev>=win(1) & ts_ev<=win(2));
    ...
    case 'erp_all_PPC'
        type = 'all'
        ts_move = monk(1).erp.PPC.sess(1).lfps(1).trialtype.(type).events.move.erp_time; ts_move_win = ts_move(ts_move>=-0.5 & ts_move<=0.5);
        ts_target = monk(1).erp.PPC.sess(1).lfps(1).trialtype.(type).events.target.erp_time; ts_target_win = ts_target(ts_target>=-0.5 & ts_target<=0.5);
        ts_stop = monk(1).erp.PPC.sess(1).lfps(1).trialtype.(type).events.stop.erp_time; ts_stop_win = ts_stop(ts_stop>=-0.5 & ts_stop<=0.5);
        ts_reward = monk(1).erp.PPC.sess(1).lfps(1).trialtype.(type).events.reward.erp_time; ts_reward_win = ts_reward(ts_reward>=-0.5 & ts_reward<=0.5);
        % Mean all channels per session
        for m = 1:length(monk)
        ...
        % average across sessions
        for m = 1:length(monk) 
            clear th_v th_w bet_v bet_w erp_move_sess erp_target_sess erp_stop_sess erp_reward_sess
            for sess = 1:length(monk(m).sess)
                erp_move_sess(sess,:) =  monk(m).sess(sess).erp_move_mu_ch; ...
                    [max_move_sess(sess,:),indx_move] = max(abs(monk(m).sess(sess).erp_move_mu_ch(1,ts_move>=-0.5 & ts_move<=0.5))); max_move_time(sess,:) = ts_move_win(indx_move);
                erp_target_sess(sess,:) = monk(m).sess(sess).erp_target_mu_ch; ...
    ...
    case 'erp_reward_densities_PFC'
        type = 'all' % Only Schro has recordings in PFC for now (Apr 2020)
