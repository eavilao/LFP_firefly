...
if electrode_id ~= 0
    ...
    nlfps = length(lfps);
    switch plot_type       
           case 'LFParray'  % For Ody..other monks need adjust 
...
%                 l = nanmean(lfps(ii+48).stats.trialtype.all.events.stop.all_freq.lfp_align,2); % PPC
        ...
        case 'sim_coherence_dist_all'
            ...
            set(gca,'xlim',[1 24], 'ylim', [0.95 1],'xTick',[1 23], 'TickDir', 'out', 'FontSize', 20); box off
            title('Coherence across distance 6-12 Hz MST'); xlabel('electrode distance'); ylabel('coherence')
            
            
            % PPC
            dist_ppc =  pop_lfps.stats.trialtype.all.PPC.dist; coher_ppc =  pop_lfps.stats.trialtype.all.PPC.coherByDist; phase_ppc = pop_lfps.stats.trialtype.all.PPC.phaseByDist; freq = pop_lfps.stats.trialtype.all.crosslfp.freq;
            %plot coher
            cmap = jet(88);
            figure; hold on; for k=1:88, plot(freq,coher_ppc(:,k),'Color',cmap(k,:)); end  
        ...
        case 'sim_coherence_dist_rew'
            ...
            set(gca,'xlim',[2 50], 'ylim', [-0.02 0.045], 'TickDir', 'out', 'FontSize', 20); box off
        ...
        case 'sim_coherence_dist_density'
            ...
            set(gca,'xlim',[2 50], 'TickDir', 'out', 'FontSize', 20); ylim([0.6 1]); box off
            title('Coherence MST reward'); xlabel('frequency'); ylabel('coherence');
            % plot difference
            figure; hold on; plot(freq,(coher_mst_low - coher_mst_high),'Color',[0.5 0.5 0.5]);
            plot(freq,nanmean(coher_mst_low,2) - nanmean(coher_mst_high,2),'Color','k', 'LineWidth',2);
            set(gca,'xlim',[2 50], 'TickDir', 'out', 'FontSize', 20); % ylim([-0.02 0.02]); box off
            title('Coherence MST reward diff'); xlabel('frequency'); ylabel('coherence');
            
            figure; hold on; for k=1:24, plot(freq,phase_mst_low(:,k),'Color','b'); end
            for k=1:24, plot(freq,phase_mst_high(:,k),'Color','m'); end
            set(gca,'xlim',[2 50], 'ylim', [-0.35 0.3], 'TickDir', 'out', 'FontSize', 20); box off
            title('Phase MST reward'); xlabel('frequency'); ylabel('rad');
            ...
            set(gca,'xlim',[1 24],'xTick',[1 23], 'TickDir', 'out', 'FontSize', 20); ylim([0.6 1]); box off
            title('Coherence across distance 6-12 Hz MST err'); xlabel('electrode distance'); ylabel('coherence')
            ...
            plot(freq,nanmean(coher_ppc_low,2) - nanmean(coher_ppc_high,2),'Color','k', 'LineWidth',2);
        ...
        case 'sim_coherence_betw_areas_reward'
            % across areas  --  area 1 is MST, area 2 is PPC.
            %% MST to PPC
            freq = pop_lfps.stats.trialtype.all.crosslfp.freq;
            coherMSTPPC_unrew = pop_lfps.stats.trialtype.density(1).MSTPPC.coherByElectrode; coherMSTPPC_rew_std = std(coherMSTPPC_unrew');  % unrew
            coherMSTPPC_rew = pop_lfps.stats.trialtype.density(2).MSTPPC.coherByElectrode; coherMSTPPC_unrew_std = std(coherMSTPPC_rew'); % rew
            
            figure; hold on; plot(freq,smooth(nanmean(coherMSTPPC_unrew,2)),'k');
            plot(freq,smooth(nanmean(coherMSTPPC_rew,2)),'g');
            set(gca,'xlim',[2 50], 'TickDir', 'out', 'FontSize', 20); box off; ylim([0.65 0.72]);
            title('MST --> <PPC> reward')
            
            figure; hold on;
            shadedErrorBar(freq, smooth(nanmean(coherMSTPPC_unrew,2),3), coherMSTPPC_unrew_std, 'lineprops', 'k');
            shadedErrorBar(freq, smooth(nanmean(coherMSTPPC_rew,2),3), coherMSTPPC_rew_std, 'lineprops', 'g');
            set(gca,'xlim',[2 50], 'TickDir', 'out', 'FontSize', 20); box off; ylim([0.58 0.68]);
            title('MST --> <PPC> reward')
            
            % Plot diff
            figure; hold on; 
            plot(freq,coherMSTPPC_unrew-coherMSTPPC_rew,'Color',[0.5 0.5 0.5]);
            plot(freq,smooth(nanmean(coherMSTPPC_unrew,2),3)- smooth(nanmean(coherMSTPPC_rew,2),3),'k','LineWidth',2);
            set(gca,'xlim',[2 50], 'TickDir', 'out', 'FontSize', 20);hline(0,'r'); box off
            title('MST --> <PPC> reward diff'); xlabel('frequency'); ylabel('coherence unrew-rew');
            
            % PPC to MST
            freq = pop_lfps.stats.trialtype.all.crosslfp.freq;
            coherPPCMST_unrew = pop_lfps.stats.trialtype.density(1).PPCMST.coherByElectrode; coherPPCMST_rew_std = std(coherMSTPPC_unrew');  % unrew
            coherPPCMST_rew = pop_lfps.stats.trialtype.density(2).PPCMST.coherByElectrode; coherPPCMST_unrew_std = std(coherMSTPPC_rew'); % rew
            
            figure; hold on; plot(freq,smooth(nanmean(coherPPCMST_unrew,2)),'k');
            plot(freq,smooth(nanmean(coherPPCMST_rew,2)),'g');
            set(gca,'xlim',[2 50], 'TickDir', 'out', 'FontSize', 20); box off; ylim([0.57 0.68]);
            title('PPC --> <MST> reward')
            
            figure; hold on;
            shadedErrorBar(freq, smooth(nanmean(coherPPCMST_unrew,2),3), coherPPCMST_unrew_std, 'lineprops', 'k');
            shadedErrorBar(freq, smooth(nanmean(coherPPCMST_rew,2),3), coherPPCMST_rew_std, 'lineprops', 'g');
            set(gca,'xlim',[2 50], 'TickDir', 'out', 'FontSize', 20); box off; ylim([0.58 0.68]);
            title('PPC --> <MST> reward')
            
            % Plot diff
            figure; hold on; 
            plot(freq,coherPPCMST_unrew-coherPPCMST_rew,'Color',[0.5 0.5 0.5]);
            plot(freq,smooth(nanmean(coherPPCMST_unrew,2),3)- smooth(nanmean(coherPPCMST_rew,2),3),'k','LineWidth',2);
            set(gca,'xlim',[2 50], 'TickDir', 'out', 'FontSize', 20);hline(0,'r'); box off
            title('PPC --> <MST> reward diff'); xlabel('frequency'); ylabel('coherence unrew-rew');
