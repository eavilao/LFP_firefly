function LFPpop_sim_no_eye(exp)
...
% % save
% disp('Saving... . . .')
% save('exp_out_lfp_stats_pop_Bruno_42_Schro_107_113_Quigley_188_185_move_before_after','exp', '-v7.3');
%save('exp_out_lfp_stats_pop_reAnalyzed_behav_just Bruno','exp', '-v7.3');
% % save('exp_out_lfp_stats_just_behav_Bruno','exp', '-v7.3');
%%
avg_monks = 1;
doPop = 0;
doCSD = 0;
...
if avg_monks
    clear all_m
    for i=1:length(monks), area_m{i} = fieldnames(monk(i).spec.area); end  % if size(area_m{i},1) == 1,  end
    areas = intersect(string(area_m{1}), string(area_m{3})); % hardocoded for now, only look at Quigley(44) and Schro(51)
    for a = 1:length(areas)
        for type = 1:length(trialtype)
            nconds = length(monk(1).spec.area.(areas{a}).(trialtype{type})); clear cond
            for cond = 1:nconds
                for ev = 1:length(events)
...
%% avg between sessions for each monkey
if doPop
    monks = unique([exp.monk_id]); events = fieldnames(exp(1).area.PPC.lfps.stats(1).trialtype.all.events);
                ...
                for ev = 1:length(events)
                    ...
                    for coh_ar = 1:length(coh_areas)
                        if (string(trialtype(type)) == 'reward') && (cond == 2)
                            monk(i).coher.trialtype.(trialtype{type})(cond).events.(events{ev}).(coh_areas{coh_ar}).coher_mu = squeeze(nanmean(coh_ev(coh_ar),1));
                            monk(i).coher.trialtype.(trialtype{type})(cond).events.(events{ev}).(coh_areas{coh_ar}).coher_phi_mu = squeeze(nanmean(coh_phi,1));
                            monk(i).coher.trialtype.(trialtype{type})(cond).events.(events{ev}).(coh_areas{coh_ar}).coher_ts = squeeze(nanmean(coh_ts,1));
                            monk(i).coher.trialtype.(trialtype{type})(cond).events.(events{ev}).(coh_areas{coh_ar}).coher_freq = squeeze(nanmean(coh_freq,1));
                        elseif ~(string(trialtype(type)) == 'reward')
                            monk(i).coher.trialtype.(trialtype{type})(cond).events.(events{ev}).(coh_areas{coh_ar}).coher_mu = squeeze(nanmean(coh_ev,1));
                            monk(i).coher.trialtype.(trialtype{type})(cond).events.(events{ev}).(coh_areas{coh_ar}).coher_phi_mu = squeeze(nanmean(coh_phi,1));
                            monk(i).coher.trialtype.(trialtype{type})(cond).events.(events{ev}).(coh_areas{coh_ar}).coher_ts = squeeze(nanmean(coh_ts,1));
                            monk(i).coher.trialtype.(trialtype{type})(cond).events.(events{ev}).(coh_areas{coh_ar}).coher_freq = squeeze(nanmean(coh_freq,1));
                        end
...
for ii = 2  %Bruno 1:length(monks)
    m = [exp.monk_id] == monks(2); p_monk = exp(m);
    for j = 1:length(p_monk)
        if doPop
            monk(ii).pop(j) = p_monk(j).pop;
        end
...
for ii = 3  %Schro  1:length(monks)
    m = [exp.monk_id] == monks(3); p_monk = exp(m);
    for j = 1:length(p_monk)
        if doPop
            monk(ii).pop(j) = p_monk(j).pop;
        end
...
save('lfp_pop_sim_no_norm_behav_move_before_after', 'monk', 'all_monks', '-v7.3');
