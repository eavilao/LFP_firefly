...
%% compute spectrogram for each trial for all channels (average all channels per trial) and per channel aligned to target onset
                    ...
                    if ~isempty(lfps(1).stats.trialtype.(trialtypes{type})(cond).events.(gettuning{ev}).lfp_align)
                        for j = 1:length(lfps(1).stats.trialtype.(trialtypes{type})(cond).events.(gettuning{ev}).lfp_align(1,:)) % trials
                            ...
                            for n = 1:length(ar)  % first 24 ch for MST if applicable
                                lfp_trl_area(n,:) =  lfps(ar(n)).trials(:,j).lfp;  % extract lfp for all ch per trial  % get # of trials (samp x ch) 334xch
                                % compute spectrogram for each trial for each channel
                                [stats.trialtype.(trialtypes{type})(cond).area.(unique_brain_areas{area}).ch(n).trl(j).spectrogram, stats.trialtype.(trialtypes{type})(cond).area.(unique_brain_areas{area}).ch(n).trl(j).ts_spectrogram,...
                                    stats.trialtype.(trialtypes{type})(cond).area.(unique_brain_areas{area}).ch(n).trl(j).freq_spectrogram] = ...
                                    mtspecgramc(lfp_trl_area(n,:),prs.spectrogram_movingwin,spectralparams);
                            end
                            % compute spectrogram
                            [stats.trialtype.(trialtypes{type})(cond).area.(unique_brain_areas{area}).pop_trl(j).spectrogram, stats.trialtype.(trialtypes{type})(cond).area.(unique_brain_areas{area}).pop_trl(j).ts_spectrogram,...
                                stats.trialtype.(trialtypes{type})(cond).area.(unique_brain_areas{area}).pop_trl(j).freq_spectrogram] = ...
                                mtspecgramc(lfp_trl_area',prs.spectrogram_movingwin,spectralparams);
                            
                            %                     figure; imagesc((stats.trialtype.(trialtypes{type})(cond).area.(unique_brain_areas{area}).pop_trl(j).ts_spectrogram)-1,stats.trialtype.(trialtypes{type})(cond).area.(unique_brain_areas{area}).pop_trl(j).freq_spectrogram,...
                            %                           stats.trialtype.(trialtypes{type})(cond).area.(unique_brain_areas{area}).pop_trl(j).spectrogram'); axis xy;
                            %                     set(gca, 'ylim',[0 50])
                        end
                    end
                end
...
% %% Compute coherence for each trial for all channels (average all channels per trial) and per channel aligned to movement stop
% if prs.compute_coherogram_whole_trial
%     fprintf('**********Computing coherogram for the whole trial between LFPs********** \n');
%     fprintf(['Time:  ' num2str(clock) '\n']);
%     spectralparams.tapers = prs.spectrum_tapers;
%     spectralparams.Fs = 1/dt;
...
%                             a1 = find(unitindx1);
%                             a2 = find(unitindx2);
%                             for n = 1:length(ar1)  % first 24 ch for MST if applicable
%                                 lfp_trl_area1(n,:) =  lfps(a1(n)).trials(:,j).lfp;  % extract lfp for all ch per trial  % get # of trials (samp x ch) 334xch
%                                 
%                                 for  n = 1:length(ar2)
%                                 lfp_trl_area2(n,:) =  lfps(a2(n)).trials(:,j).lfp;
%                                 
%                                 
%                                 % compute coherogram for each trial for each channel
%                                 [stats.trialtype.(trialtypes{type})(cond).area.([unique_brain_areas{area2} unique_brain_areas{area1}]).ch(n).trl(j).coher, stats.trialtype.(trialtypes{type})(cond).area.([unique_brain_areas{area2} unique_brain_areas{area1}]).ch(n).trl(j).coherPhi,...
%                                     stats.trialtype.(trialtypes{type})(cond).area.([unique_brain_areas{area2} unique_brain_areas{area1}]).ch(n).trl(j).crossSpec,~,~, stats.trialtype.(trialtypes{type})(cond).area.([unique_brain_areas{area2} unique_brain_areas{area1}]).ch(n).trl(j).coher_ts...
%                                     stats.trialtype.(trialtypes{type})(cond).area.([unique_brain_areas{area2} unique_brain_areas{area1}]).ch(n).trl(j).coher_freq] = ...
%                                     cohgramc(lfp_trl_area1(n,:),lfp_trl_area2(n,:),prs.spectrogram_movingwin,spectralparams);
%                             end
%                             % compute coherogram per trial for all channels
%                             [stats.trialtype.(trialtypes{type})(cond).area.([unique_brain_areas{area2} unique_brain_areas{area1}]).pop_trl(j).coher, stats.trialtype.(trialtypes{type})(cond).area.([unique_brain_areas{area2} unique_brain_areas{area1}]).pop_trl(j).coherPhi,...
%                                 stats.trialtype.(trialtypes{type})(cond).area.([unique_brain_areas{area2} unique_brain_areas{area1}]).pop_trl(j).crossSpec,~,~, ...
%                                 stats.trialtype.(trialtypes{type})(cond).area.([unique_brain_areas{area2} unique_brain_areas{area1}]).pop_trl(j).coher_ts,...
%                                 stats.trialtype.(trialtypes{type})(cond).area.([unique_brain_areas{area2} unique_brain_areas{area1}]).pop_trl(j).coher_freq] = ...
%                                 cohgramc(lfp_trl_area1', lfp_trl_area2',prs.spectrogram_movingwin,spectralparams);
%                             
%                             %                     figure; imagesc((stats.trialtype.(trialtypes{type})(cond).area.(unique_brain_areas{area}).pop_trl(j).ts_spectrogram)-1,stats.trialtype.(trialtypes{type})(cond).area.(unique_brain_areas{area}).pop_trl(j).freq_spectrogram,...
%                             %                           stats.trialtype.(trialtypes{type})(cond).area.(unique_brain_areas{area}).pop_trl(j).spectrogram'); axis xy;
%                             %                     set(gca, 'ylim',[0 50])
    ...
    for area = 1:num_brain_areas
        ...
        for ch = 1:length(ar)  % first 24 ch for MST if applicable
            for ntrl = 1:length(find(corr_indx))
                stats.area.(unique_brain_areas{area}).band_pass.theta.chan(ch).corr.trl(ntrl).ch_95th_ts = lfps(ar(ch)).stats.band_passed.ts(lfps(ar(ch)).stats.band_passed.trl(corr_trl(ntrl)).theta_95_indx); % extract timing of value (this is the important val)
                stats.area.(unique_brain_areas{area}).band_pass.beta.chan(ch).corr.trl(ntrl).ch_95th_ts = lfps(ar(ch)).stats.band_passed.ts(lfps(ar(ch)).stats.band_passed.trl(corr_trl(ntrl)).beta_95_indx); % extract timing of value(this is the important val)
            end
            ...
            [stats.area.(unique_brain_areas{area}).band_pass.theta.corr.rate_95(ch,:),stats.area.(unique_brain_areas{area}).band_pass.theta.corr.ts_rate_95(ch,:)] = ...
                BandPassedAmp2Rate(stats.area.(unique_brain_areas{area}).band_pass.theta.chan(ch).corr.trl, [-1:prs.temporal_binwidth:1], prs.temporal_binwidth);
             [stats.area.(unique_brain_areas{area}).band_pass.beta.corr.rate_95(ch,:),stats.area.(unique_brain_areas{area}).band_pass.beta.corr.ts_rate_95(ch,:)] = ...
                BandPassedAmp2Rate(stats.area.(unique_brain_areas{area}).band_pass.beta.chan(ch).corr.trl, [-1:prs.temporal_binwidth:1], prs.temporal_binwidth);
            
            %% incorr
            for ntrl = 1:length(find(incorr_indx))
                stats.area.(unique_brain_areas{area}).band_pass.theta.chan(ch).incorr.trl(ntrl).ch_95th_ts = lfps(ar(ch)).stats.band_passed.ts(lfps(ar(ch)).stats.band_passed.trl(incorr_trl(ntrl)).theta_95_indx);
                stats.area.(unique_brain_areas{area}).band_pass.beta.chan(ch).incorr.trl(ntrl).ch_95th_ts = lfps(ar(ch)).stats.band_passed.ts(lfps(ar(ch)).stats.band_passed.trl(incorr_trl(ntrl)).beta_95_indx); % extract timing of value
            end
             ...
             [stats.area.(unique_brain_areas{area}).band_pass.theta.incorr.rate_95(ch,:),stats.area.(unique_brain_areas{area}).band_pass.theta.incorr.ts_rate_95(ch,:)] = ...
                BandPassedAmp2Rate(stats.area.(unique_brain_areas{area}).band_pass.theta.chan(ch).incorr.trl, [-1:prs.temporal_binwidth:1], prs.temporal_binwidth);
             [stats.area.(unique_brain_areas{area}).band_pass.beta.incorr.rate_95(ch,:),stats.area.(unique_brain_areas{area}).band_pass.beta.incorr.ts_rate_95(ch,:)] = ...
                BandPassedAmp2Rate(stats.area.(unique_brain_areas{area}).band_pass.beta.chan(ch).incorr.trl, [-1:prs.temporal_binwidth:1], prs.temporal_binwidth);
